...
import {BankModalEditPage} from '../bancos/bank-modal-edit'
...
export class BancosPage {
  ...
  openEdit(id){
    let IdBank = id;
    let modal = this.modalCtrl.create(BankModalEditPage, IdBank);
    modal.present();
    modal.onDidDismiss(() => this.ionViewWillEnter())
  ...
  Popup(bank) {
    let alert = this.alertCtrl.create({
      title: 'Delete',
      message: '¿Quiere borrar este banco? Al intentar borrarlo si hay apoderados u oficinas asignadas a este banco la entrada no desaparecerá.',
      buttons: [
        {
          text: 'Cancel',
          role: 'cancel',
