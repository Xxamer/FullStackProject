...
import {OfficeModalEditPage} from './office-modal-edit';
/**
...
export class OficinasPage {
  ...
  openModal() {
    let modal = this.modalCtrl.create(OfficeModalPage);
    modal.present();
  ...
  openEdit(id) {
    let IdOffice = id;
    let modal = this.modalCtrl.create(OfficeModalEditPage, IdOffice);
    modal.present();
    // refresh data after modal dismissed
    modal.onDidDismiss(() => this.ionViewWillEnter())
  }
